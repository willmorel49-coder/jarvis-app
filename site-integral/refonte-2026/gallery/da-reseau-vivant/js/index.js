...
// RÉSEAU VIVANT — Intégral Pharma. We only run the formations used by this
// art direction (header + the three network-assembly scenes). The other demo
// grids are intentionally absent from the markup, so their functions are not
// called (avoids null lookups) while the effect engine stays untouched.
const init = () => {
  // prefers-reduced-motion → keep the final, already-assembled state.
  // Skipping the timelines leaves every element at its natural grid position
  // (the scatter is defined via gsap.from()), which is exactly the end frame.
  const reduceMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
  if (reduceMotion) return;

  animateFrame();      // header parallax
  animateFirstGrid();  // relay nodes rise into the national mesh
  animateFourthGrid(); // scattered points converge into the 9-agences formation
  animateSeventhGrid();// the three values assemble (masked reveal)
};
