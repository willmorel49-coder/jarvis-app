...
// REDUCED MOTION
// ------------------------------------------------------------
// When the visitor asked their OS to reduce motion, we skip every
// scroll-driven animation and leave the manifesto as plain, readable text.
...
// ------------------------------------------------------------
// Enables smooth scrolling (motion path only)
...
// ------------------------------------------------------------
if (prefersReducedMotion) {
  // Static, fully legible fallback: every line at full opacity, no blur.
  document.body.classList.add('reduced-motion');
  gsap.set(textElements, { opacity: 1, filter: 'blur(0px)' });
  gsap.set(relatedItems, { opacity: 1, scale: 1, xPercent: 0 });
} else {
  initFlips();
  initScramble();
  initRelatedDemos();
