...
(function () {
  ...
  // 25/09/2026 — To do list : un document privé en FICHIER, pour le joindre à un mail (session requise).
  V2.docProtegeFichier = function (cle) {
    var d = DOCS_PROTEGES[cle], c = (V2.sb && V2.sb()) || null;
    if (!d || !c || !c.storage) return Promise.resolve(null);
    return c.storage.from(SEAU_PROTEGE).download(d.fichier).then(function (r) {
      return (r && r.data && !r.error) ? new File([r.data], d.fichier, { type: d.type === 'pdf' ? 'application/pdf' : (r.data.type || 'application/octet-stream') }) : null;
    });
  };
  ...
  V2.chargerScripts = function (urls) {
    ...
    var V = '?v=20260925zc' + (window.V2_VER || '20260915g');
