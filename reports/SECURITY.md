...
**3 actions urgentes** :
1. Faire rotation immédiate du mot de passe Offilog `***RETIRE***` (réutilisé 2 scripts → compromis dans l'historique git).
...
| 3 | `import_wml_supabase.py` | 12-13 | Idem clés Supabase + ligne 15 `PASSWORD = '***RETIRE***'` (compte demo CRM) | **CRITIQUE** | Voir A.2 |
| 4 | `scraper_bestsellers_offilog.py` | 23-24 | `***RETIRE*** / `***RETIRE***` (compte tiers Offilog) | **CRITIQUE** | Voir A.2 |
| 5 | `scraper_offilog_live.py` | 25-26 | `***RETIRE*** / `***RETIRE***` (idem, dupliqué) | **CRITIQUE** | Voir A.2 |
| 6 | `ROBOT.md` | 46-47 | Référence à la clé anon Supabase + crédentials demo | INFO | OK (documentation) |
| 7 | `docs/superpowers/plans/2026-04-30-crm-v2-supabase.md` | 277-279 | Mots de passe **anciens** (`***RETIRE***`, `***RETIRE***`, `***RETIRE***`) | FAIBLE | Vérifier qu'ils ne sont plus actifs côté Supabase |

### A.2 Justification CRITIQUE

- **Mot de passe `***RETIRE***`** (compte `demo@integralpharma.fr`) committé en clair dans `import_wml_supabase.py`. Si le repo est public sur GitHub, toute personne avec ce fichier peut se connecter au CRM et accéder à toutes les pharmacies (RLS ouvre l'accès à tout user authentifié — voir Section E).
- **`***RETIRE*** / `***RETIRE***`** : compte Offilog tiers. Mot de passe trivial, réutilisé probablement ailleurs. Hors RGPD (compte service), mais sa fuite est une violation des CGU Offilog et un risque de poursuite IP/contractuel.
...
|---|--------|----------|--------|-------|
| 1 | **Rotation mot de passe Offilog** `***RETIRE*** (compromis dans `scraper_bestsellers_offilog.py` et `scraper_offilog_live.py`) ; charger via env var Python `os.environ['OFFILOG_PWD']` | CRITIQUE | 15 min | J+0 |
