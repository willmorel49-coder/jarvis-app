...
charge('wml-officines-data.js');
// wml-ventes-NN.js : autant de fichiers que le dépôt en contient réellement, jamais un
// compte figé — le 03/09/2026 le découpage est passé de 10 à 38 fichiers (grande passe
// conditions commerciales) et un total en dur aurait silencieusement tronqué les ventes.
...
//
// ⚠️ Ce test lisait autrefois `p.rota` (rotation annuelle) directement sur PROD_STATS.
// Depuis la « grande passe conditions commerciales » (commit dda303b0, 03/09/2026),
// PROD_STATS public ne porte plus ni rotation ni marge ni net : ces chiffres vivent
// dans prod-stats-conditions.js, hors dépôt, servi par adresse signée (absent de cette
// copie de travail). `p.rota` valait donc toujours `undefined`, l'index restait VIDE,
// et positionMarche() rendait null faute de tout périmètre commun.
// On reconstruit vM depuis les VRAIES ventes (WML_SALES), exactement comme le fait
// cipIndex() en production : demande = somme des quantités positives par produit,
// ramenée au nombre de mois couverts par le fichier.
const PS = win.PROD_STATS;
const dP = win.WML_D_PRODUITS || {};
const dem = {};
let nMoisVentes = 0;
if (win.WML_SALES && win.WML_SALES.length) {
  const mois = new Set();
  for (const s of win.WML_SALES) {
    const q = s[4] || 0;
    if (q <= 0) continue;
    mois.add(s[1]);
    const cip = String(dP[s[3]] || '');
    if (!cip) continue;
    dem[cip] = (dem[cip] || 0) + q;
  }
  nMoisVentes = mois.size || 1;
}
assert.ok(nMoisVentes > 0, 'aucune vente réelle chargée (wml-ventes-*.js) — vM resterait à 0 partout');

const idx = {};
for (const p of PS) {
  const totalDemande = dem[String(p.c)] || 0;
  const vM = totalDemande / nMoisVentes;
  if (p.c && vM > 0) idx[String(p.c)] = { c: String(p.c), d: p.d, vM, st: 0 };
}
