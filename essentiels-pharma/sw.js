...
const CACHE_NAME = 'essentiels-v4-' + new Date().toISOString().slice(0,10);
const PRECACHE = [
  './',
  './index.html',
  './style.css',
...
const NETWORK_FIRST = [
  /\/essentiels-pharma\/?$/,                 // navigation racine
  /\/essentiels-pharma\/index\.html$/,
  /\/essentiels-pharma\/app\.js(\?|$)/,
];
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    caches.keys().then((keys) => {
      return Promise.all(
        keys.filter(k => k.startsWith('essentiels-') && k !== CACHE_NAME)
  ...
  event.respondWith(
    caches.match(req).then((cached) => {
      ...
      return fetch(req).then(res => {
        if (res && res.status === 200 && (req.url.includes('/essentiels-pharma/') || req.url.includes('fonts.gstatic.com') || req.url.includes('cdn.jsdelivr.net'))) {
          const clone = res.clone();
          caches.open(CACHE_NAME).then(c => c.put(req, clone));
        }
        return res;
